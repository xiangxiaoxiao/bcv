...
int main() { 
    ...
    vector<float> img = bcv::bcv_imread<float>(in_fname, &rows, &cols);
    ...
    fftwf_free(in_fftw);
    fftwf_free(out_fftw);    
    bcv::bcv_imwrite(out_fname1, img, rows, cols);
    // ----------------------------------------------------------------------//
    ...
    transform(img.begin(), img.end(), img.begin(),
                  bind1st(multiplies<float>(),255.0/maxval ));
    fftwf_destroy_plan(p);
    bcv::bcv_imwrite(out_fname2, img, rows, cols/2+1);
    // ------------------------------------------------------------------------
    ...
    fftwf_destroy_plan(p);
    fftwf_free(out_fftw);    
    bcv::bcv_imwrite(out_fname3, img, rows, cols);
    return 0;
}
