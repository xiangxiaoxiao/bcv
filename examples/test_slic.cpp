...
int main(int argc, char** argv) {
    ...
    double t1, t2;
    int rows, cols, chan;
    vector<uchar> img = bcv_imread<uchar>(FLAGS_input.c_str(), &rows, &cols, &chan);

    Slic slic = Slic(img, rows, cols, chan, 
    ...
    bcv_imwrite<uchar>("out.png", matout, rows, cols, chan);
    printf("Writing visualization of segmentation to out.png\n");
    
    vector<SlicNode<uchar> > graph = construct_slic_graph<uchar>(img, sp, rows, cols, chan); 
    vector<uchar> avg_img = slic_average_image(graph, rows, cols, chan);
    bcv_imwrite<uchar>("out_avg.png", avg_img, rows, cols, chan);
